        ...
        for l in self.model.layers:
            ...
            for i,c in enumerate(l.weights):
                if np.random.rand() <= 0.2:
                    l.weights[i,:] = c * (np.random.randint(0,3) * np.random.randn(*c.shape) ) 
            ...
            if np.random.rand() <= 0.2:
                l.bias = l.bias * (np.random.randint(0,3) * np.random.randn(*l.bias.shape) )
    ...
    def cross(self, s1 ):
        for i,l in enumerate(self.model.layers):
            ...
            for j,c in enumerate(l.weights):
                ...
                if np.random.rand() <= 0.2:
                    l.weights[j,:], s1.brain.model.layers[i].weights[j] = s1.brain.model.layers[i].weights[j], c
